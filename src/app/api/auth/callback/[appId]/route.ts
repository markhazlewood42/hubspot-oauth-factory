...
// This file handles GET requests to /api/auth/callback/[appId] endpoint.
// The file path directly maps to the URL path
export async function GET(request: Request) {

  // TODO: Get app ID from path param and make sure it's been registered.
  // This will be a unique oauth callback URLfor each registered app.
  ...
  try {
    ...
    const tokenData = await hubspot.exchangeForTokens(code, redirectUri);

    // Get the HubSpot account info using the official client
    const accountData = await hubspot.getAccountInfoFromAccessToken(
      tokenData.accessToken
    );

    // Store the installation info
    const storeResult = await hubspot.storeInstall(
      accountData.hubId,
      tokenData.accessToken,
    ...
    return NextResponse.redirect(
      new URL(`/auth/success?portalId=${accountData.hubId}`, request.url)
    );
  } catch (error) {
